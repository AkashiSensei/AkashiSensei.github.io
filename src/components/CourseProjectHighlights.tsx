...
export function CourseProjectHighlights() {
  ...
  return (
    <section id="course-projects" className="flex w-full max-w-5xl flex-col gap-3 pt-2 sm:pt-4">
      <div className="flex flex-col gap-1.5 px-2 sm:px-4">
        <AppLink to="/course-projects" className="group flex w-fit items-center gap-2">
      ...
      <div className="relative left-1/2 flex w-screen -translate-x-1/2 snap-x snap-mandatory gap-3 overflow-x-auto px-6 pb-2 scroll-px-6 sm:px-[max(1.5rem,calc((100vw-64rem)/2+1rem))] sm:scroll-px-[max(1.5rem,calc((100vw-64rem)/2+1rem))]">
        {featuredCourseProjects.map((project) => (
          <ProjectCard
            key={project.id}
            project={project}
