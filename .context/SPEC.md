...
- Site shell should include a shared footer across public routes. The footer must stay simple, restrained, mobile-safe, and consistent with the active frosted-glass visual system; it should provide stable public navigation/contact/context affordances without duplicating dense page content or adding private information.
...
- User-controlled animation preference is first-class for high-cost decorative motion. The navigation/menu surface should expose an animation on/off control; when disabled, expensive background animation canvases/effects must stop mounting or running while the current frosted-glass cards, blur surfaces, theme, locale, routing, and static background treatment remain intact. Keep the preference persistent and structured so future animation levels can be introduced without redesigning the setting. For visitors without a saved preference, default to the low-animation/static mode rather than full effects; continue honoring any valid mode the visitor explicitly saved.
...
- Display modes (`full`, `static`, `plain`) change layout and atmosphere, not the semantic emphasis of shared content. Plain mode is a document layout: it must not strip listing-page personal-work highlight color or inline bold, and listing body copy should not read larger than the equivalent non-plain listing.
...
- Homepage FPV PageUp/PageDown steps between the existing FPV screens. From the last FPV screen, PageDown continues to the document bottom so the shared footer is fully visible. PageUp from that footer position returns to the previous FPV screen (the fourth of five), not the last FPV screen. The on-screen screen-nav control stays limited to the FPV screens and must not add a footer target.
- When `/resume` is in the desktop-landscape full-screen section layout, PageUp/PageDown must move exactly to the previous/next full-screen section, analogous to the homepage FPV screen-stepping keys. After the last resume section, PageDown may reach page end so the shared footer stays reachable; PageUp from that footer returns to the last resume section. In plain display mode, portrait/tall workspaces, and mobile, keep native browser paging because those surfaces have no full-screen page concept.
...
- On module listing pages, bullet previews use one shared cap across display modes: if an entry does not split project-intro vs personal-work, show at most 3 bullets; if it does, show at most 2 from each section. Workbench listing pages show every bullet because that module has no detail page.
...
- Root homepage may use a cinematic scroll-controlled video / frame-sequence background as its primary visual treatment: page scroll progress maps to media time or frame position, so forward/backward scrolling advances/reverses the visual timeline. This experience must stay route-scoped to `/`, keep the concise intro and entry CTAs readable, respect the site animation preference and `prefers-reduced-motion`, and provide a static or low-cost fallback for mobile, unsupported browsers, or expensive media playback. Do not couple the root homepage media timeline to `/resume` or detail-page background effects.
...
- **Listing-card image galleries**: On listing pages (`/projects`, `/course-projects`, `/knowledge`, `/tools`, and equivalent summary cards), card image strips must not capture trackpad or wheel gestures for left/right image switching. Page scrolling must continue through the image area. Cards should still auto-cycle gallery images. Image switching by the user belongs in the large-image preview (thumbnails, keyboard, and preview-strip swipe). Detail-page mobile galleries may keep a horizontal swipe strip.
